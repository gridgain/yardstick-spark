...
#  
from collections import namedtuple
Line = namedtuple('LLine', 'tstamp tname nrecs nparts nskew native xform action duration count')
...
points = ax.plot(xarr,yarr, linestyle='-', marker=markers[x%len(markers)], label=statsMap[stat],color=color[0])
ax.set_title(title,color= 'r' if host == 'i386' else 'b')
ax.set_xlabel(' ')
