...
import org.yardstick.spark.reports.ReportsDataPrep.{MapSeriesMap, Coords, SeriesMap}
...
class CoreChartingApp extends BaseChartingApp {
  val Title = "IgniteRDD Runtime Performance Compared with Native Spark RDD Ten Workers AWS c3.2xlarge Cluster"
  val useLogAxis = true
  override val xAxisLabel = "Number of Records (Thousands)"
  override val yAxisLabel = "Elapsed Time (msecs) (Lower is Better)"
  def getData(dataPath: String): (String, MapSeriesMap) = {
    val rawLines = ReportsDataPrep.grabData(dataPath)
    val lines = rawLines.map{LLine(_)}

    val (csvData, overSeriesMap) = SqlReportsDataPrep.formatData(lines)
    (csvData, overSeriesMap)
  }
}

class SqlChartingApp extends BaseChartingApp {
  val Title = "IgniteSQLRDD Runtime Performance Compared with Native Spark RDD Three Workers AWS c3.2xlarge Cluster"
  val useLogAxis = true
  override val xAxisLabel = "Number of Records (Thousands)"
  override val yAxisLabel = "Elapsed Time (msecs) (Lower is Better)"
  def getData(dataPath: String): (String, MapSeriesMap) = {
...
abstract class BaseChartingApp extends Application {
...
object ChartingApp {
  ...
  def main(args: Array[String]) {
    ...
    Application.launch(clazz.asInstanceOf[Class[_ <: Application]], args.slice(1, args.length): _*)
    pr("launched")
  }
